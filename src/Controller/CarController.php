...
//  #[Security("is_granted('USER_EDIT', usr)", message: 'Nemas pristup', statusCode: 403)]
  public function formReservation(CarReservation $reservation, Car $car, UploadService $uploadService, Request $request): Response {
    if (!$this->isGranted('ROLE_USER')) {
      return $this->redirect($this->generateUrl('app_login'));
    }
    $type = $request->query->getInt('type');
    ...
    if ($request->isMethod('POST')) {
      ...
      if ($form->isSubmitted() && $form->isValid()) {
        ...
        $uploadImages = $request->files->all()['car_reservation_form']['image'];
...
//  #[Security("is_granted('USER_EDIT', usr)", message: 'Nemas pristup', statusCode: 403)]
  ...
  public function stopReservation(CarReservation $reservation, UploadService $uploadService, Request $request): Response {
    if (!$this->isGranted('ROLE_USER')) {
      return $this->redirect($this->generateUrl('app_login'));
    }
    ...
    if ($request->isMethod('POST')) {
      ...
      if ($form->isSubmitted() && $form->isValid()) {
        ...
        $uploadImages = $request->files->all()['car_stop_reservation_form']['image'];
        if (!empty ($uploadImages)) {
          foreach ($uploadImages as $uploadFile) {
            $path = $reservation->getCar()->getUploadPath();
            $pathThumb = $reservation->getCar()->getThumbUploadPath();
...
//  #[Security("is_granted('USER_VIEW', usr)", message: 'Nemas pristup', statusCode: 403)]
  ...
  #[Entity('reservation', expr: 'repository.findForFormCar(car)')]
//  #[Security("is_granted('USER_EDIT', usr)", message: 'Nemas pristup', statusCode: 403)]
  public function formEmployeeReservationDetails(CarReservation $reservation, Car $car, UploadService $uploadService, Request $request): Response {
    if (!$this->isGranted('ROLE_USER')) {
      return $this->redirect($this->generateUrl('app_login'));
    }
    ...
    if ($request->isMethod('POST')) {
      ...
      if ($form->isSubmitted() && $form->isValid()) {
        ...
        $uploadImages = $request->files->all()['car_reservation_form_details']['image'];
        if (!empty ($uploadImages)) {
          foreach ($uploadImages as $uploadFile) {
            $path = $reservation->getCar()->getUploadPath();
            $pathThumb = $reservation->getCar()->getThumbUploadPath();
  ...
  public function stopEmployeeReservationDetails(CarReservation $reservation, UploadService $uploadService, Request $request): Response {
    if (!$this->isGranted('ROLE_USER')) {
      return $this->redirect($this->generateUrl('app_login'));
    }
    ...
    if ($request->isMethod('POST')) {
      ...
      if ($form->isSubmitted() && $form->isValid()) {
        ...
        $uploadImages = $request->files->all()['car_stop_reservation_form_details']['image'];
        if (!empty ($uploadImages)) {
          foreach ($uploadImages as $uploadFile) {
            $path = $reservation->getCar()->getUploadPath();
            $pathThumb = $reservation->getCar()->getThumbUploadPath();
...
//  #[Security("is_granted('USER_EDIT', usr)", message: 'Nemas pristup', statusCode: 403)]
  public function formEmployeeReservation(CarReservation $reservation, UploadService $uploadService, Request $request): Response {
    if (!$this->isGranted('ROLE_USER')) {
      return $this->redirect($this->generateUrl('app_login'));
    }
  ...
  public function stopEmployeeReservation(CarReservation $reservation, UploadService $uploadService, Request $request): Response {
    if (!$this->isGranted('ROLE_USER')) {
      return $this->redirect($this->generateUrl('app_login'));
    }
    $mobileDetect = new MobileDetect();
