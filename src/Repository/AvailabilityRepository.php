...
class AvailabilityRepository extends ServiceEntityRepository {
  ...
  public function getReport(array $data, Company $company): array {
    ...
    if (!empty($data['zaposleni'])) {
      ...
      $users = $this->getEntityManager()->getRepository(User::class)->findBy(['company' => $company, 'userType' => UserRolesData::ROLE_EMPLOYEE, 'isSuspended' => false]);
    }
  ...
  public function getDostupnostByUser(User $user): array {
    ...
    if (!empty($dostupnosti)) {
      foreach ($dostupnosti as $dost) {
        ...
        if ($dost->getDatum()->format('N') < $user->getCompany()->getSettings()->getWorkWeek()) {
          $dostupnost[] = [
