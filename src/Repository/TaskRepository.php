...
class TaskRepository extends ServiceEntityRepository {
    ...
    public function __construct(ManagerRegistry $registry, Security $security, ParameterBagInterface $parameterBag, MailService $mail) {
...
//    dd($lista);
    ...
    public function getUsersByDateExing(DateTimeImmutable $date, $project): array  {
...
//        'logStatus' => $this->getEntityManager()->getRepository(TaskLog::class)->getLogStatus($tsk)
    ...
    public function sendDailyEmail(Task $task): Task {

        $today = (new \DateTimeImmutable())->setTime(0, 0, 0);
        $tomorrow = (new \DateTimeImmutable())->setTime(23, 59, 59);


        $createdAt = $task->getDatumKreiranja();
        ...
        $signEmail = $this->getEntityManager()->getRepository(SignEmail::class)->createQueryBuilder('s')
            ->where('s.project = :projectId')
            ->andWhere('s.created >= :today')
            ->andWhere('s.created <= :tomorrow')
        ...
        if (!is_null($signEmail)) {

            $listaKorisnika = $signEmail->getUsers();
            $listaKorisnikaNova = $signEmail->getUsers();
            foreach ($task->getAssignedUsers()->toArray() as $assign) {

                if (is_array($listaKorisnikaNova) && !in_array($assign->getId(), $listaKorisnikaNova)) {
                    $listaKorisnikaNova[] = $assign->getId();
                }

            }

            $this->getEntityManager()->getRepository(SignEmail::class)->save($signEmail->setUsers($listaKorisnikaNova));

            if (!empty(array_diff($listaKorisnikaNova, $listaKorisnika)) || !empty(array_diff($listaKorisnika, $listaKorisnikaNova))) {

                $project = $task->getProject();

                if ($project->getId() == 214) {
                ...
                $users = $this->getUsersByDateExing($danas, $project);
                ...
                if ($project->getId() == 214) {
                    if (is_dir($urbanPath)) {
                        ...
                        foreach ($usersList as $usr) {
                            $atch = $this->getEntityManager()->getRepository(Pdf::class)->findOneBy(['user' => $usr, 'title' => 'Dokumenti']);
                            if (!is_null($atch)) {
                    ...
                    if (!empty($usersList)) {
                        $this->mail->reportsDailyExing('marceta.pars@gmail.com', $project->getClient()->first(), $danas, $attachments, $projectList);
                        $this->mail->reportsDailyExing('zoran.stankovic@exing.co.rs', $project->getClient()->first(), $danas, $attachments, $projectList);
                        $this->mail->reportsDailyExing('radisa.baltic@exing.co.rs', $project->getClient()->first(), $danas, $attachments, $projectList);
                        $this->mail->reportsDailyExing('petar.petrovic@exing.co.rs', $project->getClient()->first(), $danas, $attachments, $projectList);
                        $this->mail->reportsDailyExing('sanja.petronijevic@exing.co.rs', $project->getClient()->first(), $danas, $attachments, $projectList);
                    }
                    // Briši fajlove
