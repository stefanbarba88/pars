...
//    $task2->getTaskLogs()->clear();
        ...
        $sql = "UPDATE " . $em->getClassMetadata(Comment::class)->getTableName() . " 
                  SET task_id = :task1 WHERE task_id = :task2 ";
        ...
        $sql = "UPDATE " . $em->getClassMetadata(Overtime::class)->getTableName() . " 
                  SET task_id = :task1 WHERE task_id = :task2 ";
        ...
        $sql = "DELETE FROM " . $em->getClassMetadata(TaskHistory::class)->getTableName() . "
                WHERE task_id = :task2 ";
...
//        'logStatus' => $this->getEntityManager()->getRepository(TaskLog::class)->getLogStatus($tsk)
    ...
    public function createTaskFromChecklist(ManagerChecklist $checklist): Task {
        ...
        $task->setPhase($checklist->getPhase());
...
//    }
    ...
    public function save(Task $task): Task {
        ...
        if (!is_null($task->getPhase())) {
            $this->getEntityManager()->getRepository(Phase::class)->calculatePhaseEfficiency($task->getPhase());
        }
...
//      ->andWhere('t.status = 3')
    ...
    public function countTasksPhase(Phase $phase): array {
        ...
        foreach ($tasks as $task) {
            ...
            if ($task->getStatus() === TaskStatusData::VERIFIKOVANO) {
                $doneCount++;
            }
        }
